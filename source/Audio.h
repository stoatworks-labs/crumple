...
/**
	...
	Copied from millpond, which copied it from rosette, which adapted it from
	macroblock's analyser -- the one that has been driven with real music
	through Resolume's FFT buffer. Crumple needs the full-range level (Audio
	Scrunch adds it to the crumpling) and the onset detector (a kick scrunches
	harder), including the
	...
	**Normalised against its own recent peak**, so the same Audio Scrunch means
	the same scrunch on a quiet stem and a mastered track. The cost is that a
	long loud passage reads as "1" throughout, because relative to the last
	few seconds it is.
*/
namespace crumple::audio
{
