...
plugin's own shaders, so something has to enforce it. `crtest` drives the real
...
Nothing here checks the PORTED half: `Controls.cpp`, all of `Sheet.cpp` and
the frame sequence of `ProcessOpenGL` are hand translations in plugin.js, and
only a reader can tell whether they still agree. Nor does it see
`withoutNoperspective()`, the one load-time change the page makes to the
sheet pass's text; plugin.js says why that change is exact.
"""
import os
import re
import sys
...
SHADERS = [
    ...
    ("SHEET_VERTEX", "kSheetVertexShader"),
    ("SHEET", "kSheetShader"),
    ("STRAIN", "kStrainShader"),
    ("SOURCE", "kSourceShader"),
    ("FFT", "kFFTShader"),
    ("SOLVE", "kSolveShader"),
    ("COMPOSITE", "kCompositeShader"),
]

# No pass is assembled at run time: every shader is one whole string.
ASSEMBLY = []
