...
Everything is live on the plugin's own defaults -- a crumpled, part-flattened
sheet under a lamp from the top left -- except the one control that needs
audio: Audio Scrunch adds the level to Crumple, so it is swept with a beat
fed into the Audio buffer. The table was checked by emptying it: Audio
Scrunch alone goes dead.
...
# Applied to every render. Nothing: the defaults crumple the card.
BASE = []

# What else has to be true for a parameter to be able to do anything, as raw
# harness arguments.
CONTEXT = {
    # Crumple sits at 0.85 by default, so the scrunch has room to add to it.
    "Audio Scrunch": ["--beat", "--frames", "40"],
}
...
def render(harness, out, settings, raw, verbose):
    ...
    if result.returncode != 0:
        raise RuntimeError(f"crtest failed: {result.stderr.strip()}")
...
def parameters(harness):
    ...
    if result.returncode != 0:
        raise RuntimeError(f"crtest --list failed: {result.stderr.strip()}")
...
def main():
    ...
    harness = REPO / args.build / "crtest"
    if not harness.exists():
        print(f"no crtest at {harness} -- build first", file=sys.stderr)
        return 2
