...
# own grep that always runs. `flat` in particular is a natural name in a
# plugin about a flat sheet -- the first draft used it -- and `sample`,
# `filter`, `patch` and `layout` are just as easy to reach for.
#
...
# Shaders the plugin assembles at run time. Crumple has none: every pass is
...
if [[ -f demo/tools/check_shaders.py ]]; then
	if python3 demo/tools/check_shaders.py >/tmp/crumple-demo-shaders.log 2>&1; then
		echo "ok   $( tail -1 /tmp/crumple-demo-shaders.log )"
	else
		tail -12 /tmp/crumple-demo-shaders.log
		fail "the demo's shaders have drifted -- see /tmp/crumple-demo-shaders.log"
	fi
else
...
step "Bundle"
#---------------------------------------------------------------------------
bundle="$BUILD/Crumple.bundle"
binary="$bundle/Contents/MacOS/Crumple"
...
echo "ok   Crumple: $arches, plugMain exported"
...
[[ "$identifier" == "com.stoatworks.ffgl.crumple" ]] || fail "bundle id is '$identifier'"
[[ "$executable" == "Crumple" ]] || fail "CFBundleExecutable is '$executable'"
...
if [[ -x "$OXBOW" ]]; then
	...
	printf '%s\n' "$probe" | sed 's/^/   /'
	grep -q 'CR01' <<<"$probe" || fail "oxbow did not read the id CR01"
	grep -q 'SW Crumple' <<<"$probe" || fail "oxbow did not read the name 'SW Crumple'"
	grep -qi 'effect' <<<"$probe" || fail "oxbow did not read the type as an effect"
	echo "ok   a host reads CR01 / SW Crumple / effect"
else
...
"$BUILD/crtest" --flat
"$BUILD/crtest" --isometry
"$BUILD/crtest" --lambert
"$BUILD/crtest" --shadow
"$BUILD/crtest" --monotone
